export class CourtyardAudio {
  ...
  async enable(){if(this.enabled)return true;const Context=window.AudioContext||window.webkitAudioContext;if(!Context)throw new Error('当前浏览器无法播放庭院音景。');if(!this.context)this.create(Context);await this.context.resume();this.enabled=true;this.gain.gain.setTargetAtTime(.075,this.context.currentTime,.8);this.chime();return true;}
  disable(){if(!this.enabled)return false;this.enabled=false;this.gain?.gain.setTargetAtTime(0,this.context.currentTime,.35);return false;}
